...
def build_prompt(question, context_chunks):
    ...
    prompt = f"""You are an AI assistant with expertise in cybersecurity policy, particularly the NIST SP 800 series and their application. You can answer technical questions related to cybersecurity policy, respond to cybersecurity scenarios, and respond to general inquiries. You are also capable of engaging in casual conversation when appropriate.
                ...
                Output as your answer only the following query and the response you would give to it. Format the two in JSON format, as in "query" and "response" being the JSON keys. Both should just be simple strings in markdown formatting. Make sure to include them both, in the format {{\n\t"query":INSERT FOLLOWING QUERY HERE,\n\t"response":INSERT YOUR RESPONSE HERE\n}}
                ### Query:
                {question}
...
if __name__ == "__main__":
    with open("trainingQuestions.txt", 'r') as f:
        qs = f.readlines()
...
#                    print(output)
                    ...
                    with open(f"../faiss-training-7-{llm.replace(':','_')}/{llm.split(':')[0]}-training-questions-answer.txt", 'a') as f:
                        f.write(output)
                    time.sleep(0.05)
