...
def build_prompt(question, context_chunks):
    ...
    prompt = f"""You are an AI assistant with expertise in cybersecurity policy, particularly the NIST SP 800 series and their application. You can answer technical questions related to cybersecurity policy, respond to cybersecurity scenarios, and respond to general inquiries. You are also capable of engaging in casual conversation when appropriate.
                ...
                Output as your answer only the following query and the answer you would give to it in JSON format, with "query" and "response" as the JSON keys. Both should just be simple strings.
                ### Query:
                {question}
...
#                    print(f"\nAnswer from model: {llm} and file: {file}. Time: ({total_time}): {answer}\n")
                    output = f"{answer}\n\n"
                    if not os.path.exists(f"../faiss-training-6-{llm.replace(':','_')}"):
                        os.makedirs(f"../faiss-training-6-{llm.replace(':','_')}")
                    with open(f"../faiss-training-6-{llm.replace(':','_')}/{llm.split(':')[0]}-training-questions-answer.txt", 'a') as f:
                        f.write(output)
                    time.sleep(0.05)
