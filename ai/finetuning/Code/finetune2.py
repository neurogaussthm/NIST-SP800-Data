...
def finetune(model):
    datasetTrain = datasets.load_dataset("json", data_files=f"../faiss-training-7-{model}_2b/{model}-clean-all-2.jsonl")
    datasetTest = datasets.load_dataset("json", data_files=f"../faiss-training-7-{model}_2b/{model}-clean-test-2.jsonl")
    ...
    output_dir = f"../faiss-training-7-{model}_2b/epochresults-2"
    training_args = TrainingArguments(
        output_dir=output_dir,
        logging_dir=f"../faiss-training-7-{model}_2b/logs-2",
        learning_rate=2e-6,
        weight_decay=0.01,
    ...
    model_base.save_pretrained(f"../faiss-training-7-{model}_2b/baseoutput-2")
    tokenizer.save_pretrained(f"../faiss-training-7-{model}_2b/baseoutput-2")
    trainer.save_model(f"../faiss-training-7-{model}_2b/finaloutput-2")
    tokenizer.save_pretrained(f"../faiss-training-7-{model}_2b/finaloutput-2")
...
def mergeModel(model):
    ...
    tokenizer = AutoTokenizer.from_pretrained(f"../faiss-training-7-{model}_2b/finaloutput-2")
    peft_model = PeftModel.from_pretrained(base_model, f"../faiss-training-7-{model}_2b/finaloutput-2")

    merged_model = peft_model.merge_and_unload()
    merged_model.save_pretrained(f"../faiss-training-7-{model}_2b/merged-2")
    tokenizer.save_pretrained(f"../faiss-training-7-{model}_2b/merged-2")
