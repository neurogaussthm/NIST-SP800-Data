            ...
            for model in EMBEDDING_MODELS:
                ...
                for q in qs:
                    if q[0] != '#':
                        ...
                        output = f"{q}\n\n"
                    if not os.path.exists(f"../faiss-responses-6-{llm.replace(':','_')}"):
                        os.makedirs(f"../faiss-responses-6-{llm.replace(':','_')}")
                    with open(f"../faiss-responses-6-{llm.replace(':','_')}/q0-answer{file.split('.')[0]}.txt", 'a') as f:
                        f.write(output)
                    time.sleep(0.05)
